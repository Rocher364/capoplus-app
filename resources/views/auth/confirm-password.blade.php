...
<body class="bg-slate-100 min-h-screen flex items-center justify-center px-4">
    <div class="w-full max-w-sm bg-white rounded-2xl border border-slate-300 shadow-md p-8">
        ...
        <h1 class="text-xl font-bold text-slate-900 tracking-tight mb-1">Confirmez votre mot de passe</h1>
        ...
        <form method="POST" action="{{ route('password.confirm.store') }}" class="space-y-5">
            @csrf
            <div>
                <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">Mot de passe</label>
