...
<div class="max-w-2xl">
    <div class="mb-6">
        ...
        <p class="text-sm font-medium text-slate-600 mt-0.5">Un numÃ©ro de membre unique sera gÃ©nÃ©rÃ© automatiquement.</p>
    </div>
    ...
    <form action="{{ route('members.store') }}" method="POST" id="memberForm" class="bg-white rounded-2xl border border-slate-300 shadow-md overflow-hidden">
        ...
        <div class="px-6 py-4 bg-slate-100 border-b border-slate-300">
            <h3 class="text-xs font-bold text-slate-700 uppercase tracking-wider">IdentitÃ©</h3>
        </div>
        <div class="p-6 space-y-5 border-b border-slate-200">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">PrÃ©nom <span class="text-rose-500">*</span></label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="sexe" class="mt-1.5 w-full rounded-xl border @error('sexe') border-rose-500 @else border-slate-300 @enderror bg-slate-50 text-slate-900 text-sm px-4 py-2.5 focus:bg-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition outline-none">
                        ...
                        <option value="F" @selected(old('sexe') === 'F')>FÃ©minin</option>
        ...
        <div class="px-6 py-4 bg-slate-100 border-b border-slate-300">
            <h3 class="text-xs font-bold text-slate-700 uppercase tracking-wider">CoordonnÃ©es</h3>
        </div>
        <div class="p-6 space-y-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">TÃ©lÃ©phone <span class="text-slate-400 font-normal">(Optionnel)</span></label>
...
<script>
    // Anpeche doub-klik lÃ¨ w ap soumÃ¨t fÃ²m lan
