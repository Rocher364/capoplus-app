    ...
    <div class="flex min-h-screen">
        <aside class="hidden md:flex w-64 bg-slate-900 text-slate-100 flex-col shrink-0 no-print">
            ...
            <nav class="flex-1 px-3 py-4 space-y-1">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition {{ request()->routeIs('dashboard') ? 'bg-orange-600 text-white' : 'text-slate-300 hover:bg-slate-800' }}">Tableau de bord</a>
                <a href="{{ route('reports.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition {{ request()->routeIs('reports.*') ? 'bg-orange-600 text-white' : 'text-slate-300 hover:bg-slate-800' }}">Rapports</a>
                <a href="{{ route('accounts.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition {{ request()->routeIs('accounts.*') ? 'bg-orange-600 text-white' : 'text-slate-300 hover:bg-slate-800' }}">Comptes</a>
                <a href="{{ route('members.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition {{ request()->routeIs('members.*') ? 'bg-orange-600 text-white' : 'text-slate-300 hover:bg-slate-800' }}">Membres</a>
                <a href="{{ route('loans.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition {{ request()->routeIs('loans.*') ? 'bg-orange-600 text-white' : 'text-slate-300 hover:bg-slate-800' }}">Prets</a>
            </nav>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            <header class="app-header bg-white border-b px-4 sm:px-8 py-3 sm:py-4 flex items-center justify-between no-print">
                        ...
                        <nav class="absolute left-0 top-11 z-30 w-56 rounded-xl border border-slate-200 bg-white p-2 shadow-xl">
                            ...
                            <div class="border-t border-gray-100 mt-2 pt-2">
                                <form action="{{ route('logout') }}" method="POST">
                                    @csrf
                                    <button type="submit" class="w-full text-left px-4 py-2 text-sm font-semibold text-red-600 hover:bg-red-50 rounded-md transition block">Se deconnecter</button>
                                </form>
                            </div>
                        </nav>
                    </details>
                ...
                <div class="app-header-tools flex items-center gap-2 sm:gap-3 shrink-0">
                    <span class="app-clock text-xs text-slate-400" id="cash-clock"></span>
                    <button data-action="refresh" type="button" class="min-h-11 flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 text-xs font-medium hover:bg-slate-50 cursor-pointer">
